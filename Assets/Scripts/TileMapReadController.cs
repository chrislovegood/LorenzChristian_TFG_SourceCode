...
public class TileMapReadController : MonoBehaviour
{
    ...
    public TileBase GetTileOcean(Vector3Int gridPosition)
    {

        if(oceanTilemap == null)
        {
            oceanTilemap = GameObject.Find("OceanTilemap").GetComponent<Tilemap>();
        }

        if(oceanTilemap == null){ return null; }

        TileBase tile = oceanTilemap.GetTile(gridPosition);
